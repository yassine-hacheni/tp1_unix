...
int main() {
    ...
    while(!arret_client){
        ...
        printf("\n[CLIENT] Appuyez sur Ctrl+C pour annuler\n\n");
        ...
        if (n < 0 || n > NMAX) {
            printf("[CLIENT] Erreur: le nombre doit être entre 1 et %d\n", NMAX);
            close(fd1);
            close(fd2);
            exit(1);
        }else if(n == 0){
        ...
        printf("[CLIENT] (Ctrl+C pour annuler)\n");
        ...
        // Recherche du PID du serveur
        // Note: Dans un cas réel, le serveur devrait communiquer son PID
        // Ici, on utilise une approche simplifiée avec pgrep ou un fichier
        
        // Méthode simple: le serveur écrit son PID dans un fichier
        // Pour cet exercice, on suppose que le serveur a été lancé juste avant
        // et on peut utiliser ps/pgrep pour le trouver
